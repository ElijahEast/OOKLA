...
router.patch(
  ...
  async (req, res, next) => {
    ...
    try {
      const result = await pool.query(
        `UPDATE friendships
         ...
         WHERE id = $2
           ...
           AND status = 'pending'
         RETURNING *,
           (SELECT username FROM users WHERE id = requester_id) AS requester_username`,
      ...
      if (action === 'accepted') {
        await Promise.all([
          awardXP(req.user.sub, {
            eventType: 'friend_accepted',
            xp: 10,
            description: 'New friend!',
          }),
          awardXP(result.rows[0].requester_id, {
            eventType: 'friend_accepted',
            xp: 10,
